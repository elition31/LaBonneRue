...
$LastPost = $pdo->query("
SELECT i.id, i.title, i.description, c.title AS category, i.picture AS picture, q.title AS quality, u.user_name AS owner 
FROM items i 
...
if (!empty($_POST) && isset($_SESSION['auth'])) {
	...
	if ($CheckLock == NULL || !empty($CheckLock)) {
		...
		header("Location: include/view_annonce.php");
		exit();
	}else{
